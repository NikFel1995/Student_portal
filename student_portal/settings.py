...
SECRET_KEY = 'django-insecure-x%wrdqb=2s_4^cqz@621s(035fu9ay+^*8y4oys-x!*=for0+7'
...
STATIC_URL = 'static/'  # URL-адрес, используемый для ссылки на статические файлы, расположенные в STATIC_ROOT
STATIC_ROOT = BASE_DIR / 'assets/static'  # Абсолютный путь к каталогу, в котором collectstatic статические файлы собираются для развертывания.
...
LOGIN_REDIRECT_URL = 'classroom:index'  # перенаправление пользователя после успешного входа
LOGOUT_REDIRECT_URL = 'classroom:index'  # перенаправление пользователя после успешного выхода
